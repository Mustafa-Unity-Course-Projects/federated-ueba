...
def calculate_error_stats(model, dataloader, features): # Changed val_loader to dataloader
    """Calculates error statistics (mean and std per feature) from the provided dataloader using MSE."""
    model.eval()
    all_feature_errors = []
    with torch.no_grad():
        for batch in dataloader:
            ...
            output = model(batch)
            # Use MSE for error calculation to match anomaly detection logic
            batch_feature_error = torch.mean((output - batch)**2, dim=1) # (B, D)
            all_feature_errors.append(batch_feature_error.cpu().numpy())

    all_feature_errors = np.concatenate(all_feature_errors, axis=0)
    
    # Removed TRIMMED STATS logic to match federated approach
    mean_per_feature = np.mean(all_feature_errors, axis=0)
    std_per_feature = np.std(all_feature_errors, axis=0)
...
def train_centralized():
    ...
    for epoch in range(EPOCHS):
        ...
        # Calculate error stats using the TRAIN loader (to match federated)
        current_error_stats = calculate_error_stats(model, train_loader, features) # Changed val_loader to train_loader
    ...
    summary_data = {
        ...
        "window_size": int(task.WINDOW_SIZE), # Added window_size
        "top_k_ratio": float(TOP_K_FEATURES), # Added top_k_ratio
