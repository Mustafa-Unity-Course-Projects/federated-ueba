...
JOBLIB_CACHEDIR = "./joblib_cache"
DATA_PATH = "./dataset/r4.2"

memory = Memory(JOBLIB_CACHEDIR, verbose=0)
...
def get_u_features_dicts(ul, data='r5.2'):
    ...
    for f in list_uf:
        ul[f] = ul[f].fillna("UNKNOWN").astype(str)
        tmp = list(set(ul[f]))
        tmp.sort()
...
if __name__ == "__main__":
    ...
    for i in ["tmp", "ExtractedData", "DataByWeek", "NumDataByWeek"]:
    ...
    os.makedirs(original_dir + "/ExtractedData/", exist_ok=True)
    for mode in ['week', 'day', 'session']:
        ...
        all_csv = open(original_dir + '/ExtractedData/' + mode + dname + '.csv', 'a')
        ...
        if mode == 'session' and len(subsession_mode) > 0:
            for k1 in subsession_mode:
                for k2 in subsession_mode[k1]:
                    all_csv = open(original_dir + '/ExtractedData/' + mode + k1 + str(k2) + dname + '.csv', 'a')
