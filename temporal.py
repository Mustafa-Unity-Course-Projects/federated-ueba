        ...
        for i in range(len(udayidx)):
            ...
            if t >= min_idx:
                ...
                if calc_type == 'meddiff':
                    ...
                    percentile_i = [percentileofscore(window[:, j], udata[i, j], 'mean') - 50 for j in
                                    range(window.shape[1])]
...
def subtract_percentile_combination(data, dtype, calc_type='percentile', window_size=7, dname='cert', parallel=True):
    ...
    combined_data = pd.DataFrame(np.vstack([return_dict[ri] for ri in return_dict.keys()]),
                                 columns=combining_features + ['org_' + f for f in keep_org_features] + info_features)
...
if __name__ == "__main__":
    ...
    print(
        'If "too many opened files", or "ForkAwareLocal" error, run ulimit command, e.g. "$ulimit -n 10000" to increase the limit first')
